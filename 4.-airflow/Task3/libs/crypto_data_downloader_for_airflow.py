...
class CryptoDataDownloader:
    def __init__(self, coin_id, date, api_key=None):
        self.coin_id = coin_id
        self.date = date
        self.api_key = api_key
        self.formatted_date = datetime.strptime(date, "%Y-%m-%d").strftime("%d-%m-%Y")
        self.url = f"https://api.coingecko.com/api/v3/coins/{self.coin_id}/history?date={self.formatted_date}&x_cg_demo_api_key={self.api_key}"
        logging.debug(f"url con API key {self.url}")
...
class MainApplicationForAirFlow:
    ...
    def process_single_day(self, date=None, apikey=None):
        sleep(5) # para poder insertar 30 ya que el API permite 20 consecutivos
        logging.info("Esperando 5 segundos")
        if date is None:
            date = self.date
        #date = datetime.strptime(date, "%Y-%m-%d")
        downloader = CryptoDataDownloader(self.coin_id, date, apikey)
        try:
    ...
    def process_bulk_days(self, apikey=None):
        start_date = datetime.strptime(self.start_date, "%Y-%m-%d")
        end_date = datetime.strptime(self.end_date, "%Y-%m-%d")
        dates = [start_date + timedelta(days=i) for i in range((end_date - start_date).days + 1)]

        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            futures = [executor.submit(self.process_single_day, date.strftime("%Y-%m-%d"), apikey) for date in dates]
            for future in futures:
                future.result()


    def run(self, apikey=None):
        logging.debug(f"API key in run method {apikey}")
        if self.date:
            self.process_single_day(apikey=apikey)
        elif self.start_date and self.end_date:
            self.process_bulk_days(apikey=apikey)
        else:
...
if __name__ == "__main__":
    ...
    app = MainApplicationForAirFlow(coin_id="bitcoin", start_date="2023-12-22", end_date="2024-01-21", db=True)
    app.run(apikey=api_key)

# if __name__ == "__main__":
#     api_key = "CG-FzSij3qa7YXK18Kzfsn6xA3v"  
#     app = MainApplicationForAirFlow(coin_id="bitcoin", date="2024-01-21", db=True)
#     app.run(apikey=api_key)