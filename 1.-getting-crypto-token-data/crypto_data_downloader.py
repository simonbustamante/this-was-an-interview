...
class MainApplication:
    def __init__(self):
        ...
        parser.add_argument("--coin-id", type=str, help="Identifier of the cryptocurrency (e.g., bitcoin)")
        parser.add_argument("--date", type=str, help="Date in ISO8601 format (YYYY-MM-DD) for single day processing")
        parser.add_argument("--start-date", type=str, help="Start date in ISO8601 format (YYYY-MM-DD) for bulk processing")
        parser.add_argument("--end-date", type=str, help="End date in ISO8601 format (YYYY-MM-DD) for bulk processing")
        parser.add_argument("--workers", type=int, default=3, help="Number of concurrent workers")
        parser.add_argument("--db", action="store_true", help="Store data in the PostgreSQL database")
        self.args = parser.parse_args()
        self.engine = create_engine('postgresql://simon:bustamante@localhost:5432/simon')
    ...
    def process_single_day(self, date=None):
        if date is None:
            date = self.args.date
        #date = datetime.strptime(date, "%Y-%m-%d")
        downloader = CryptoDataDownloader(self.args.coin_id, date)
        try:
            ...
            if self.args.db:
                ...
                self.save_data_to_db(self.args.coin_id, date, data)
    ...
    def run(self):
        ...
        elif self.args.start_date and self.args.end_date:
